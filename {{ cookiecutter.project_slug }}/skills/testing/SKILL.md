...
- Preferred target: mirror `src/{{ cookiecutter.package_name }}` structure under `tests/unit` for new or moved tests.
  - example: `src/{{ cookiecutter.package_name }}/foo/bar.py` -> `tests/unit/foo/test_bar.py`
